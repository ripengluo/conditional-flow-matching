...
class OTPlanSampler:
    ...
    def get_map(self, x0, x1): # now x0, x1 stand for the joint source and target distribution
    ...
    def sample_plan(self, x0_c, x1_c, x0_L, x1_L, sys_oh=None, replace=True): # now we are considering joint plan of both coefficient and lattice
        ...
        bs = x0_c.shape[0]
        # joint source
        x0_joint = torch.cat([x0_c.flatten(1), x0_L.flatten(1)], dim=-1)
        # joint target
        x1_joint = torch.cat([x1_c.flatten(1), x1_L.flatten(1)], dim=-1)

        pi       = self.get_map(x0_joint, x1_joint)
        i, j = self.sample_map(pi, bs, replace=replace)

        if sys_oh != None:
            return x0_c[i], x1_c[j], x0_L[i], x1_L[j], sys_oh[j]
        else:
            return x0_c[i], x1_c[j], x0_L[i], x1_L[j]
        #return x0_c[i], x1_c[j], x0_L[i], x1_L[j]

    def sample_plan_with_labels(self, x0_c, x1_c, x0_L, x1_L, y0=None, y1=None, replace=True):
        r"""Compute the OT plan $\pi$ (wrt squared Euclidean cost) between a source and a target
